...
router.post('/server/issue', async (req,res)=>{
  ...
  try {
    const resp = await fetch((process.env.PAYLINK_SELF_URL || 'https://checkout.example.com') + '/server/create-order', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ amount: rec.amount, currency: rec.currency }) });
